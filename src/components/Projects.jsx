...
const Projects = () => {
  ...
  return (
    ...
    >
      <div className="max-w-6xl p-4 mx-auto flex flex-col justify-center w-full h-full">
        ...
        <div>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-7">
            {projects.map(({ id, src, title, descr }) => (
              <div
                key={id}
                className="flex flex-col rounded-md shadow-lg bg-white bg-opacity-[0.03] drop-shadow-sm shadow-black w-full p-4"
              >
                <img src={src} alt="" className="rounded-md" />
                <div className="my-3">
                  <h3 className="text-xl font-bold">{title}</h3>
                  <p className="text-gray-400">{descr}</p>
                </div>
                <div className="flex flex-row text-sm text-gray-300">
                  {skills.map(({ id, skill }) => (
                    <span
                      key={id}
                      className="bg-gray-600 px-2 py-0.5 mr-1.5 w-fit rounded-[4px]"
                    >
                      {skill}
                    </span>
                  ))}
